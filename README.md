...
## Brief
My first project, built from the ground up primarily using vanilla JavaScript. Virus Buster is a riff on the classic game Space Invaders. The criteria was to choose a classic game from the arcades and replicate the game as a browser game using Javascript. This was a solo project lasting 9 days.



## Concept
I chose to do Space Invaders and the MVP was that the player should be able to clear at least one wave of aliens and score should be displayed at the end of the game. With extra goals being the page should be responsive, each wave gets more difficult and a persistent leaderboard saved in local storage. For the theme I went with covid-19 and vaccines to replace the aliens and players.
...
### Planning
##### Wire Frame
Before signing off, I created a wire frame of how I thought the layout would be compared to the final game, the layout has a few differences but overall kept the layout of the individual components. Some functionality like lines of enemies moving in different directions did not make it into the final game. 
![Space](https://user-images.githubusercontent.com/94964514/158641422-21f8cdd3-2881-4f4e-92cc-8edbe7c00eba.png)

##### Pseudo Code
I wrote some pseudo coding notes to give me a rough idea of the order I need to work on parts. 
...
##### Checklist
Early on in development, I figured I needed to make a checklist of things to do and to also help me visualise what order to do them in. There are a couple features I did not implement that would be added in future versions.
...
#### Day 1
I started off with making the base HTML, CSS and JavaScript files. Then making a 10 x 10 grid and finding first draft image assets for the play, enemy and the projectile. The first features to implement where players start position and movement. As per the game movement was limited to the bottom line with no need of up to down movement. Next was adding in an enemy sprite at random positions that would disappear and reappear on the right input. Then I worked on the player projectile that spawned 1 tile above the player and moved up a tile every set interval. The character sprites are displayed by changing the class of the tile to one where that background image of the sprite is set, collision was added by checking the class of the tile and if two different classes matched.      
...
#### Day 3
At the beginning of the day I fixed a visual bug where a line would reach the end and not visually go down a line and then move across. The issue was that  it looked as if they would move 1 tile diagonally in the opposite direction, this was because the set interval for moving down and across was set to the same time. To fix this I shortened  the down movement interval and then you can see them move across. The next stage was for the enemies to fire back. This was very similar to the player fire function except a random enemy is chosen to fire back anywhere in the array at set intervals. Other additions include, big visual updates, GIF effects for collisions, dynamic live counter, and others.       
...
By the end of the fourth day of coding, I ended up with this:        
...
Start of day 5 was the beginning of bug fixes, general refinements and adding extras.           
...
#### Day 6
I worked on the sound effects of the game and background music. I made a sounds bank library object and several audio 'channels' in the HTML with tags. The game over screen is now updated to show the players score and the current high score; at this stage the score is not saved in local storage.      
...
#### Day 7
This was the last full day of coding for this project. Nearly all time was spent on learning how to utilise local storage for the leaderboard. At page load, the high score array of objects (contains key/values for player name and score) is taken from local storage. Next the array is sorted by score value and in a table tag in the HTML. A new row item is appended for each score. Any time the player finishes a game they are prompted to enter the name for the leaderboard table. The table is wiped and the list is sorted again with the new obj added. The local storage is updated and the new leaderboard is created.          
...
#### Day 8
By the end of the week the game looked like this and I presented our project to the course group.     
...
- Being new to JavaScript, there were many functions and methods I was not familiar with yet like array/object spreading that would have helped a lot for certain parts of the game.
- Most bugs I came across were around the loose conditions of the enemies reaching the bottom line. The way the array of enemy positions where set in such a way that the last numbers in it where not necessarily in on the button row. Therefore the set intervals would continue to run even after the last line was reached and produced recurring errors.
- A bug that turned out to be a user error of sorts was that during early development, the start button would be highlighted after being clicked on but when the player pressed the spacebar, the start button would be pressed again. I had to disable the start button from being active when the game started to stop more than one set of set intervals from running at the same time. 
...
Project Link: [https://kumasta.github.io/SEI-Project-1-Virus-Buster/](https://kumasta.github.io/Virus-Buster/)
